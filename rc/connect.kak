...
define-command connect-shell -params 1.. -shell-completion -docstring 'Run a shell with [e]dit, [b]uffer and VISUAL, EDITOR connected to the current client' %{
  nop %sh(export PATH=$kak_opt_connect_source/commands:$kak_opt_connect_source/aliases:$PATH VISUAL=edit EDITOR=edit KAKOUNE_SESSION=$kak_session KAKOUNE_CLIENT=$kak_client; $@)
}
